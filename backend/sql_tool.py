...
- publications(id, pub_id, title, authors, year, source_publication, article_type, q_rank_scopus, q_rank_wos, impact_factor, cite_score, matched_department_id UUID FK, matched_institute_id UUID FK, matched_department TEXT, home_author_department TEXT)
...
def execute_sql(sql: str) -> Any:
    ...
    try:
        resp = _get_client().rpc("exec_sql", {"query_text": sql}).execute()
